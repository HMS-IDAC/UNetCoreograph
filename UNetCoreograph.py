...
def coreSegmenterOutput(I, initialmask, findCenter):
    ...
    nuclearMask = morphological_chan_vese(
        nucGF, 100, init_level_set=initialmask, smoothing=10, lambda1=1.001, lambda2=1
    )

    TMAmask = nuclearMask
    TMAmask = remove_small_objects(
        TMAmask > 0, round(TMAmask.shape[0]) * round(TMAmask.shape[1]) * 0.005
    )
...
if __name__ == "__main__":
    ...
    if not args.tissue:
        ...
        coreMask = binary_fill_holes(
            binary_closing(
                Iblur > threshold_otsu(Iblur), np.ones((imageblur * 2, imageblur * 2))
            )
        )
        coreMask = remove_small_objects(
            coreMask, min_size=0.001 * coreMask.shape[0] * coreMask.shape[1]
        )
        ...
        estCoreDiamX = (
            np.array([(ele.bbox[3] - ele.bbox[1]) * 1.1 for ele in P]) / dsFactor
        )
        estCoreDiamY = (
            np.array([(ele.bbox[2] - ele.bbox[0]) * 1.1 for ele in P]) / dsFactor
        )
    ...
    for iCore in range(numCores):
        ...
        maskTMA = maskTMA + cv2.resize(
            singleMaskTMA, maskTMA.shape[::-1], interpolation=cv2.INTER_NEAREST
        )
